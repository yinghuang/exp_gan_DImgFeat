...
import argparse
from model import Generator, Encoder, DiscriminatorImg
import numpy as np
import socket
...
parser.add_argument('--lambda_tv', type=float, default=0.00001, help='lambda smooth loss')
parser.add_argument('--lambda_rec', type=float, default=10, help='lambda reconstruction loss')
parser.add_argument('--lambda_cls', type=float, default=1.0, help='lambda classification loss')
...
parser.add_argument('--z_dim', type=int, default=50, help='dims of encoded features')
...
parser.add_argument('--load_epoch', type=int, default=-1, help='which epoch to load? set to -1 to use latest cached model, 0 means from new')
...
#        self._criterion_rec = rec_loss
        def cls_loss(input, target_hot):
            _, target = torch.max(target_hot, 1)
            criterion = torch.nn.CrossEntropyLoss().cuda()
            return criterion(input, target)
        self._criterion_cls = cls_loss
...
#            os.mkdir(self._save_dir)
        ...
        state = {
            ...
            'Discriminator': self._D.state_dict(),
            'Encoder': self._E.state_dict(),
            'epoch': epoch
        }
        save_path = os.path.join(self._save_dir,'epoch_%d_model.t7'%(epoch))
        torch.save(state, save_path)
        print 'saved net: %s' % save_path
    
    def _set_and_check_load_epoch(self):
        # -1 表示最新, 如果最新的不存在 则变为0
        if os.path.exists(self._save_dir):
            if self.opt.load_epoch == -1:
                load_epoch = 0
                for file in os.listdir(self._save_dir):
                ...
                for file in os.listdir(self._save_dir):
                    if file.startswith("epoch_"):
                        found = int(file.split('_')[1]) == self.opt.load_epoch
                        if found: break
    ...
    def load(self):
        if self.opt.load_epoch < 1:
            ...
            return
        load_path = os.path.join(self._save_dir,'epoch_%d_model.t7'%(self.opt.load_epoch))
        checkpoint = torch.load(load_path)
        self._G.load_state_dict(checkpoint['Generator'])
        self._D.load_state_dict(checkpoint['Discriminator'])
        self._E.load_state_dict(checkpoint['Encoder'])
        print("resume from %s"%(load_path))
...
#               torch.sum(torch.abs(mat[:, :, :-1, :] - mat[:, :, 1:, :]))) / (mat.size(2) * mat.size(3))
    ...
    def _gradinet_penalty_D(self, fake_imgs, z_prior, y):
        # interpolate sample
        ...
        interpolated_prob,_ = self._D(interpolated, z_prior)
    ...
    def _forward_D(self):
        # generate outputs
        z_img = self._E.forward(self._input_real_img)
        imgs_fake = self._G.forward(z_img, self._input_fake_y)
#        d_logits_fake_img = self._D.forward(imgs_fake.detach(), self._input_z_prior, self._input_real_y)
#        d_logits_fake_z = self._D.forward(self._input_real_img, z_img.detach(), self._input_real_y)  
#        d_logits_real = self._D.forward(self._input_real_img, self._input_z_prior, self._input_real_y)  
        d_logits_fake_img, _ = self._D.forward(imgs_fake.detach(), self._input_z_prior)
        d_logits_fake_z, _ = self._D.forward(self._input_real_img, z_img.detach())  
        d_logits_real, d_logits_real_cls = self._D.forward(self._input_real_img, self._input_z_prior)  
        
        # loss cls
        self._loss_D_cls = self._criterion_cls(d_logits_real_cls, self._input_real_y) * self.opt.lambda_cls
        
        # loss gan
        self._loss_D_fake = self._compute_loss_gan(d_logits_fake_img, is_real=False)
        self._loss_D_fake += self._compute_loss_gan(d_logits_fake_z, is_real=False)
        ...
        self._loss_D_gp = self._gradinet_penalty_D(imgs_fake, 
                                                   self._input_z_prior, 
                                                   self._input_real_y) * self.opt.lambda_D_gp
        
        # combine losses
        if self._optimize_mode == "WGAN":
            self._loss_D = self._loss_D_fake + self._loss_D_real + self._loss_D_gp + self._loss_D_cls
        else:
            self._loss_D = self._loss_D_fake + self._loss_D_real + self._loss_D_cls
    ...
    def _forward_EG(self, stage=None, keep_data_for_visuals=False):
        # generate outputs
        z_img = self._E.forward(self._input_real_img)
        imgs_fake_rec = self._G.forward(z_img, self._input_real_y)
#        imgs_fake_rec = self._G.forward(self._E.forward(imgs_fake), self._input_real_y)
        imgs_fake = self._G.forward(z_img, self._input_fake_y)
#        imgs_fake = self._G.forward(self._input_z_prior, self._input_real_y)
        d_logits_fake_img, d_logits_fake_cls = self._D.forward(imgs_fake, self._input_z_prior)
        d_logits_fake_z, _ = self._D.forward(self._input_real_img, z_img)  
        ...
        if keep_data_for_visuals:
            ...
            self._vis_fake_imgs_from_prior = self._G.forward(self._input_z_prior, self._input_fake_y)
            _, self._vis_rand_y = torch.max(self._input_fake_y, 1)
            _, self._vis_y = torch.max(self._input_real_y, 1)
        else:
            # loss cls
            self._loss_G_cls = self._criterion_cls(d_logits_fake_cls, self._input_fake_y) * self.opt.lambda_cls
        
            # loss gan
            self._loss_G_fake = self._compute_loss_gan(d_logits_fake_img, is_real=True)
            self._loss_G_fake += self._compute_loss_gan(d_logits_fake_z, is_real=True)
            ...
            # loss rec further
            self._loss_G_rec_fur = self._criterion_rec_further(imgs_fake, self._input_real_img) * self.opt.lambda_rec
            ...
            if stage == None or stage == 3:
                ...
                self._loss_G = self._loss_G_fake + self._loss_G_cls
            elif stage == 2: # rec  cls ?
                self._loss_G = self._loss_G_rec + self._loss_G_tv + self._loss_G_rec_fur + self._loss_G_cls
            return self._loss_G
...
class Train:
    def __init__(self, dataset, opt):
        ...
        print("------------model resume------------")
        self._model._set_and_check_load_epoch()
        self._model.load()
...
#==============================================================================
    ...
    def print_current_train_errors(self, epoch, i, iters_per_epoch, errors, t, visuals_were_stored):
        ...
        message_head = '%s (T(if do visual?)%s, epoch: %d/%d, it: %d/%d, t/smpl: %.3fs) ' % (log_time, visuals_info, epoch, self.opt.train_epochs, i, iters_per_epoch, t)
        message_loss = ''
...
#            log_file.write('%s\n' % message)
    ...
    def _train(self):
        for i_epoch in range(self.opt.load_epoch + 1, self.opt.train_epochs + 1):
            self._train_epoch(i_epoch)
            self._model.save_checkpoint(i_epoch)
